            ...
            {payload?.items?.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="py-2 leading-[1.4] border-b border-border font-Syne">
                    <span className="text-[14px] font-medium">
                      {config.label} ↩{" "}
                    </span>
                    <p className=" leading-[1.9] text-[12px] font-extralight font-Syne">
                      {config.description}
                    </p>
                  </div>
                  ...
                  <div
                    ...
                    className="py-2 text-center font-medium leading-[1.5]"
                  >
          ...
          <div className="flex gap-x-2">
            {payload?.items?.map((item) => {
              ...
              return (
                <div
                  key={item}
                  className="p-1 border border-border rounded-md shadow-md items-center"
                >
                  {/* Image */}
                  <img
                    src={config.image}
                  ...
                  {/* Body */}
                  <div className="p-1">
                    <div className=" leading-[1.4] font-Syne">
                      <span className="text-[14px] font-medium">
                        {config.label}
                      </span>
                      <p className=" leading-[1.9] text-text/70 text-[12px] font-extralight font-Syne">
                        {config.description}
                      </p>
                    </div>
                    ...
                    <div
                      onClick={() => sendMessage(`i want ${item}`)}
                      className="py-2 text-center font-medium leading-[1.5] w-full px-2.5 py-1.5 rounded-lg bg-text/5 border border-border text-[11px] font-medium text-text cursor-pointer hover:bg-text/10 transition-colors"
                    >
                      <button className="text-[13px] font-Syne">
                        {" "}
              ...
              >
                ...
                {provider.image ? (
                  ...
                  <div className="w-full h-[100px] bg-purple/10 flex items-center justify-center">
                    <span className="text-2xl font-semibold text-purple font-Syne">
                ...
                <div className="p-2.5 space-y-2">
                  <div className="flex items-center gap-1 bg-muted rounded-lg w-14 px-2">
                    <Star
                      size={12}
                  ...
                  {/* Name + location */}
                  <div>
                    <p className="text-[13px] font-semibold text-text leading-tight font-Syns">
                      {provider.name}
                    </p>
                    {provider.location && (
                      <p className="text-[11px] text-text mt-0.5 flex items-center gap-1">
                  ...
                  {provider.description && (
                    <p className="text-[11px] text-text leading-[1.5] line-clamp-2">
                      {provider.description}
                    </p>
                  )}
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-border">
                    ...
                    <button
                      ...
                      className="text-[11px] text-[13px] font-Syne font-medium px-5 py-2 rounded-md bg-purple/75 border border-border text-[11px] font-medium text-text cursor-pointer hover:bg-purple/80 transition-colors"
                    >
                      Book now
                    </button>
                  </div>
